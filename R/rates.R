  ...
  ddply(x, .(species, origin, virus, temperature, dpi), 
        ...
        function(dfrm){
          ...
          return(data.frame(n_total_specimens = n_total_specimens,
                            ...
                            TE = n_positive_saliva/n_total_specimens*100,
                            titre_mean = titre_mean,
                            titre_sd = titre_sd,
                            IR_out =  paste(formatC(round(n_positive_bodies/n_total_specimens*100, 2), 1, format = "f"),
                                            " (",
                                            n_positive_bodies,
                                            "/",
                                            n_total_specimens,
                                            ")", sep = ""),
                            DR_out =  paste(formatC(round(n_positive_legs/n_positive_bodies*100, 2), 1, format = "f"),
                                            " (",
                                            n_positive_saliva,
                                            "/",
                                            n_positive_bodies,
                                            ")", sep = ""),
                            TR_out =  paste(formatC(round(n_positive_saliva/n_positive_bodies*100, 2), 1, format = "f"),
                                            " (",
                                            n_positive_saliva,
                                            "/",
                                            n_positive_bodies,
                                            ")", sep = ""),
                            TE_out =  paste(formatC(round(n_positive_saliva/n_total_specimens*100, 2), 1, format = "f"),
                                            " (",
                                            n_positive_saliva,
                                            "/",
                                            n_total_specimens,
                                            ")", sep = ""),
                            titre_out = paste(formatC(round(titre_mean, 2), 1, format = "f"),
                                              " (",
                                              formatC(round(titre_sd, 2), 1, format = "f"),
                                              ")", sep = "")
          )
          )
        }
